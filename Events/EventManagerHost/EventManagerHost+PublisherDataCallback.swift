...
extension EventManagerHost {
    ...
    public func listenTo<Publisher: EventManagerHost, Data, Event: RawRepresentable where Event.RawValue == String>(_ publisher: Publisher, event: Event, callback:(Publisher, Data) -> ()){
    ...
    func wrapCallback<Publisher: EventManagerHost, Data>(_ callback: (Publisher, Data) -> ()) -> (EventPublisherData<Publisher, Data>) -> (){
        return { event in
            callback(event.publisher, event.data)
        }
    }

    func internalOn<Publisher: EventManagerHost, Data>(_ publisher: Publisher, event: String, callback: (EventPublisherData<Publisher, Data>) -> ()){
    ...
    func trigger<Publisher: EventManagerHost, Data>(_ event: EventPublisherData<Publisher, Data>){
